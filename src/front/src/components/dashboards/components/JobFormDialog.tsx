...
import { Dialog, DialogTitle, DialogContent, DialogActions, Grid, TextField, Button, Chip, FormControl, InputLabel, Select, MenuItem, Alert, RadioGroup, FormControlLabel, Radio, FormLabel, Input, Box, Typography, Divider } from '@mui/material';

interface JobFormDialogProps {
  open: boolean;
  onClose: () => void;
  onPublish: (data: jobPayload) => Promise<void>;
}
...
const JobFormDialog: React.FC<JobFormDialogProps> = (props) => {
  ...
  const handleChangeTestType = (value: 'none' | 'pdf' | 'questions') => {
    setTestType(value);
    if (value === 'none') {
      setAnexo(undefined);
      setQuestions([]);
  ...
  const addQuestion = () => {
    setQuestions([
      ...questions,
      { title: '', description: '', language: 'javascript', starterCode: '' },
    ]);
  };
            ...
            <Grid item xs={12} md={6}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  value={testType}
                  onChange={(e) => handleChangeTestType(e.target.value as any)}
                >
            ...
            <Button
              ...
              onClick={() => {
                if (testType === 'questions') {
                  ...
                  setAnexo(`json:testSpec:${JSON.stringify(spec)}`);
                }
              }}
            >
              Continuar
